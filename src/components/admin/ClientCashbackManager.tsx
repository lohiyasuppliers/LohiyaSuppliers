...
interface VariationRow {
  ...
  cashbackValuePaise: number | null;
}
...
function draftFromProducts(rows: ProductRow[]) {
  ...
  for (const p of rows) {
    ...
    if (p.cashbackValuePaise != null) {
      initial[pk] = paiseToRupees(p.cashbackValuePaise);
    }
    for (const v of p.variations) {
      const vk = `${p.id}:${v.id}`;
      if (v.cashbackValuePaise != null) initial[vk] = paiseToRupees(v.cashbackValuePaise);
    }
  }
  return initial;
}

export function ClientCashbackManager({ clientId }: { clientId: string }) {
  ...
  const loadCashback = useCallback(
    async (scope: ViewScope) => {
      setLoading(true);
      const res = await fetch(`/api/admin/users/${clientId}/cashback?scope=${scope}`);
  ...
  function setCashback(key: string, value: string) {
  ...
  async function handleSave() {
    ...
    const rules: { productId: string; variationId: string; cashbackRupees: string | null }[] = [];
    ...
    for (const key of keys) {
      ...
      if (value) {
        rules.push({ productId, variationId, cashbackRupees: draft[key] });
      } else if (hadSaved) {
        rules.push({ productId, variationId, cashbackRupees: null });
      }
    }

    const res = await fetch(`/api/admin/users/${clientId}/cashback`, {
      method: "PUT",
    ...
    if (res.ok) {
      const data = await res.json();
      alert(`Cashback saved! ${data.ruleCount} custom cashback rule(s) active.`);
      await loadCashback(viewScope);
    } else {
      alert((await res.json()).error || "Failed to save cashback");
    }
    setSaving(false);
  }
  ...
  return (
    <div className="bg-white rounded-xl border overflow-hidden">
      <div className="p-5 border-b bg-amber-50/60 flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">
            {viewScope === "custom"
              ? `Showing ${ruleCount} cashback rule(s). Same ₹/unit is auto-synced to product discount — client picks one at checkout.`
              : "Browse the full catalog to set product or variant cashback for this client."}
          </p>
        </div>
        <div className="flex flex-wrap items-center gap-2">
          {viewScope === "custom" ? (
            <button
              ...
              className="inline-flex items-center gap-2 px-4 py-2 border border-amber-200 text-amber-800 rounded-lg text-sm font-medium hover:bg-amber-50"
            >
      ...
      <div className="p-4 border-b bg-gray-50/50 space-y-3">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          >
            {viewScope === "custom" ? "Set rules only" : "Full catalog"}
          </span>
          <span className="text-xs px-2.5 py-1 rounded-full bg-amber-100 text-amber-900 font-medium">
      ...
      <div className="max-h-[32rem] overflow-y-auto divide-y">
        {filtered.map((p) => {
          ...
          const productHasRule = !!draft[productKey]?.trim() || p.cashbackValuePaise != null;

          const visibleVariations =
            viewScope === "custom"
              ? p.variations.filter(
                  (v) => draft[`${p.id}:${v.id}`]?.trim() || v.cashbackValuePaise != null
                )
              : p.variations;
              ...
              <div className="flex items-center gap-3 p-4 hover:bg-gray-50/80">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-gray-500">
                    ...
                    {productHasRule && (
                      <span className="ml-2 text-amber-700 font-medium">Product cashback set</span>
                    )}
                  </p>
                </div>
                ...
                {(viewScope === "all" || productHasRule) && (
                  <div className="w-32 shrink-0">
                    <label className="text-xs text-amber-700 block mb-0.5">Cashback ₹ / unit</label>
                    <input
                      type="number"
                      step="0.01"
                      min="0"
                      ...
                      onChange={(e) => setCashback(productKey, e.target.value)}
                      className="w-full px-2 py-1.5 border border-amber-200 rounded-lg text-sm focus:ring-2 focus:ring-amber-300"
                    />
                  </div>
                )}
              </div>
                ...
                <div className="px-4 pb-4 pl-12 space-y-2">
                  {visibleVariations.map((v) => {
                    ...
                    const variantHasRule =
                      !!draft[varKey]?.trim() || v.cashbackValuePaise != null;
                    return (
                      <div
                        key={v.id}
                      ...
                      >
                        ...
                        <div className="w-32 shrink-0">
                          <input
                            ...
                            placeholder="None"
                            value={draft[varKey] ?? ""}
                            onChange={(e) => setCashback(varKey, e.target.value)}
                            className="w-full px-2 py-1.5 border border-amber-200 rounded-lg text-sm bg-white focus:ring-2 focus:ring-amber-300"
                          />
                        </div>
                      </div>
                    );
        ...
        {filtered.length === 0 && (
          <div className="p-10 text-center text-gray-500 text-sm">
            {viewScope === "custom" ? (
              <>
                <p className="font-medium text-gray-700">No cashback rules yet</p>
                <p className="mt-1">Click &quot;Add More Cashback&quot; to set per-client rewards.</p>
              </>
            ) : (
