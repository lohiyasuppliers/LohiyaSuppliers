...
export default async function UserManagementPage() {
  ...
  const users = await prisma.user.findMany({
    include: {
      clientProfile: {
        select: { company: true, billingState: true, gstin: true, allowGstChoice: true },
      },
          ...
          <table className="w-full text-sm min-w-[1100px]">
            <thead className="bg-gray-50">
              <tr>
                ...
                <th className="text-left px-4 py-3 font-medium text-gray-600 whitespace-nowrap">Contact No.</th>
                <th className="text-left px-4 py-3 font-medium text-gray-600 whitespace-nowrap">Company</th>
                <th className="text-left px-4 py-3 font-medium text-gray-600 whitespace-nowrap">GSTIN</th>
                <th className="text-left px-4 py-3 font-medium text-gray-600 whitespace-nowrap">GST option</th>
                <th className="text-left px-4 py-3 font-medium text-gray-600 whitespace-nowrap">State</th>
                ...
                <th className="text-left px-4 py-3 font-medium text-gray-600 whitespace-nowrap">Orders</th>
                <th className="text-left px-4 py-3 font-medium text-gray-600 whitespace-nowrap">Total Spent</th>
                <th className="text-left px-4 py-3 font-medium text-gray-600 whitespace-nowrap">Joined</th>
                <th className="text-right px-4 py-3 font-medium text-gray-600 whitespace-nowrap min-w-[220px]">Actions</th>
              </tr>
            </thead>
              ...
              {users.map((user) => {
                ...
                return (
                  <tr key={user.id} className={`hover:bg-gray-50 ${!user.isActive ? "bg-red-50/40" : ""}`}>
                      ...
                      {user.role === Role.CLIENT ? (
                        <span
                          className={`inline-flex px-2 py-0.5 rounded-full text-xs font-medium ${
                            user.clientProfile?.allowGstChoice
                              ? "bg-blue-100 text-blue-700"
                              : "bg-gray-100 text-gray-600"
                          }`}
                        >
                          {user.clientProfile?.allowGstChoice ? "With / without GST" : "GST only"}
                        </span>
                      ) : (
                        "—"
                      )}
                    ...
                    <td className="px-4 py-3 font-medium text-brand-800 whitespace-nowrap">
                    ...
                    <td className="px-4 py-3 text-right">
                      <div className="inline-flex flex-col items-end gap-2 min-w-[200px]">
                        <Link
                          ...
                          className="inline-flex items-center gap-1 text-brand-600 hover:underline text-xs whitespace-nowrap"
                        >
